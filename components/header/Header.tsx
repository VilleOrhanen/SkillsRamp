...
const Header = (props: IMenuProps & { settings: ISiteSettings }) => {
  ...
  const renderDesktopNav = () => (
    <nav
      key={props._key}
      className={`duration-800 fixed top-0 z-40 hidden w-full transition delay-300 ease-in-out md:block`}>
      <div className="flex justify-between py-2">
        <Link href="/" className="z-40 flex items-center">
          <Image
            {...settings.logo}
            alt={settings.title}
            className="mx-10 max-h-12 w-12 rounded-full object-contain hover:scale-105"
          />
        </Link>
